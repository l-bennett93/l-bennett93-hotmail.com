...
def prettified_confusion(y_true,y_pred,labels, title = None):
    cm = confusion_matrix(y_true.values,y_pred,labels = labels)
    cm = pd.DataFrame(cm,index=[3,4,5,6,7,8],columns=labels)
    cm.index.name = 'Actual'
    cm.columns.name = 'Predicted'
    plt.figure(figsize = (10,7))
    if title: plt.title(f"{title}",size = 24)
